...
public class CalculateKey extends JPanel {
	 ...
	 * Create the panel.
	 */
	public CalculateKey(JFrame frame, int role, int p, int g, int privatekey, int recieved) {
		setLayout(null);
		...
		for (int i = 0; i < Integer.toBinaryString(privatekey).length(); i++) {
			...
			if (w.substring(i, i+1).equals("1")) {
				val = val*recieved; // val = (val*recieved)
				val = val%p;
			}
		...
		// Header
		JLabel header = new JLabel("Step 5: Calculate Key");
		...
		btnNextStep.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				// Visible after the calculation is complete
				if (btnNextStep.isVisible()) {
					EncryptDecrypt panel = new EncryptDecrypt(frame, out); // Opens login window
		...
		String r = ""; // opposite of who's selected
		String AorB = ""; // a for Alice's private key, b for Bob's private key
		if (role == 0) {
			AorB = "a";
			r = "b";
		}
		...
		// description
		JLabel description = new JLabel("Now to calculate K    (y_"+r+")^"+AorB+" (mod p) using modular exponentiation");
		description.setFont(textFont);
		description.setForeground(Color.WHITE);
		description.setBounds(screenWidthMiddle-400, screenHeightMiddle-230, 900, 40);
		add(description);
		...
		congruence.setBounds(screenWidthMiddle-174, screenHeightMiddle-230, 20, 40);
		add(congruence);
		
		// description
		JLabel description2 = new JLabel("Using our values from the previous step we get K    "+recieved+"^"+privatekey+" (mod "+p+")");
		...
		congruence2.setBounds(screenWidthMiddle+180, screenHeightMiddle-200, 20, 40);
		add(congruence2);		
		...
		// Pretty much same code as in KeyExchangeP1, just uses a different base really
		...
		txt_input.setBounds(screenWidthMiddle-250, screenHeightMiddle-120, 200, 30);
				...
				try {
					...
					if (output == answers[count]) {
						...
						if (compare < Integer.toBinaryString(privatekey).length()) {
							...
							previousOutput2.setFont(mathFont);
